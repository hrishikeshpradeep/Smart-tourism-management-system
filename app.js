...
const discoveryServiceGuides={
  kozhikode:{stay:['STAY IN KOZHIKODE','Kozhikode Beach, Mavoor Road or railway-station area','Compare verified stays near the part of the city you plan to explore.'],dining:['MALABAR DINING','SM Street and the Beach Road area','Look for Malabar cuisine and check current hours before you go.'],vehicles:['Taxi or radio cab','Auto rickshaw','KSRTC / private bus']}
};
...
function renderTravelAssistant(){
  ...
  const discoveryGuide=discoveryServiceGuides[String(selected?.name||'').toLowerCase()]||{stay:[`STAY NEAR ${selected?.name||'YOUR DESTINATION'}`,'Browse verified stays near your selected stops','Check location, recent reviews and cancellation terms before booking.'],dining:['LOCAL DINING','Explore nearby restaurants and regional specialities','Use current map listings for opening hours and menus.'],vehicles:['Local taxi or cab','Auto rickshaw','Public transport']};
  ...
  services.innerHTML=selected?.isDiscovery?`<div class="service-group"><span>${discoveryGuide.stay[0]}</span><strong>${discoveryGuide.stay[1]}</strong><small>${discoveryGuide.stay[2]}</small></div><div class="service-group"><span>${discoveryGuide.dining[0]}</span><strong>${discoveryGuide.dining[1]}</strong><small>${discoveryGuide.dining[2]}</small></div><div class="vehicle-options"><p>LOCAL TRANSPORT</p>${discoveryGuide.vehicles.map(vehicle=>`<button data-vehicle="${vehicle}" type="button"><span>${vehicle}</span><b>Check current fare</b></button>`).join('')}</div>`:`<div class="service-group"><span>STAY NEAR ${selected?.name||'YOUR DESTINATION'}</span><strong>${serviceCopy[0]}</strong><small>Comfortable base for your day plan</small></div><div class="service-group"><span>LOCAL DINING</span><strong>${serviceCopy[1]}</strong><small>Popular regional flavours nearby</small></div><div class="vehicle-options"><p>VEHICLE OPTIONS</p>${['Compact cab','SUV / family','Tempo traveller'].map((vehicle,index)=>`<button data-vehicle="${vehicle}" type="button"><span>${vehicle}</span><b>${index===0?serviceCopy[2]:index===1?'From ₹2,000':'From ₹3,400'}</b></button>`).join('')}</div>`;
  persistRoute();
...
async function loadTravelServices(place){
  ...
  try{
    ...
    if(!providers.length&&place.isDiscovery)return;
