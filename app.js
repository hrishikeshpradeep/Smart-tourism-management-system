const destinations = [
  ...
  {id:'coorg',name:'Coorg',region:'Karnataka',style:'wildlife',cost:3400,rating:4.6,best:'Oct – Apr',image:'https://upload.wikimedia.org/wikipedia/commons/thumb/3/30/Paddy_Hills_Inakanahalli_Coorg_Nov24_A7CR_04454.jpg/3840px-Paddy_Hills_Inakanahalli_Coorg_Nov24_A7CR_04454.jpg',summary:'Coffee country with rainforest walks, waterfalls and quiet stays.',interests:['nature','adventure','food'],attractions:['Abbey Falls','Raja’s Seat','Dubare Elephant Camp']}
];
destinations.push(
  {id:'kashmir',name:'Kashmir',region:'Jammu and Kashmir',style:'mountain',cost:4400,rating:4.9,best:'Apr - Oct',image:'https://upload.wikimedia.org/wikipedia/commons/b/b7/Kashmir%2C_aerial_view_of_snow-clad_mountains_-_20170201_161036.jpg',summary:'Lakes, meadow trails and mountain views in Indias most cinematic valley.',interests:['nature','adventure','culture'],attractions:['Dal Lake','Gulmarg','Pahalgam']},
  ...
  {id:'ooty',name:'Ooty',region:'Tamil Nadu',style:'mountain',cost:3000,rating:4.6,best:'October - June',image:'assets/place-ooty-nilgiri-train.png',summary:'Cool weather, tea slopes and scenic train rides through the Nilgiris.',interests:['nature','food','culture'],attractions:['Ooty Lake','Doddabetta Peak','Nilgiri Mountain Railway']}
);
...
const localPlaceImages = {goa:'assets/place-goa.jpg',manali:'assets/place-manali.jpg',munnar:'assets/place-munnar.jpg',varanasi:'assets/place-varanasi.jpg',kashmir:'assets/place-kashmir.jpg',ladakh:'assets/place-ladakh.jpg',hampi:'assets/place-hampi-v2.jpg',ooty:'assets/place-ooty.jpg'};
...
function planFor(id){const selected=destination(id);$('#tripDestination').value=id;setPlannerDestinationTheme(selected);closeModals();location.hash='planner';showToast(`${selected.name} added to your trip form`) }
...
$('#tripDestination').addEventListener('change',event=>setPlannerDestinationTheme(destination(event.target.value)));
async function loadDestinationsFromApi(){try{const response=await fetch(`${API_BASE_URL}/destinations`);if(!response.ok)throw new Error('Destination API unavailable');const apiDestinations=await response.json(),interestByCategory={beach:['food','nature','adventure'],mountain:['nature','adventure'],heritage:['culture','food'],wildlife:['nature','adventure'],nature:['nature','adventure'],adventure:['nature','adventure','culture'],coastal:['food','culture','nature']},savedBeforeLoad=[...wishlist];destinations.splice(0,destinations.length,...apiDestinations.map(d=>({id:d.id,slug:d.slug,name:d.name,region:`${d.city}, ${d.state}`,style:d.category,cost:Number(d.dailyCost),rating:Number(d.rating),best:d.bestSeason,image:localPlaceImages[d.slug]||d.imageUrl||imageBySlug[d.slug]||'https://images.unsplash.com/photo-1469474968028-56623f02e42e?auto=format&fit=crop&w=1800&q=88',summary:d.summary,interests:interestByCategory[d.category]||['nature'],attractions:(d.attractions||[]).map(a=>a.name)})));wishlist=[...new Set(savedBeforeLoad.map(savedId=>destinations.find(d=>d.id===savedId||d.slug===savedId)?.id||savedId))];saveWishlist();$('#tripDestination').innerHTML=destinations.map(d=>`<option value="${d.id}">${d.name}, ${d.region}</option>`).join('');renderDestinations();if(session)loadWishlist()}catch(error){console.warn('Using local destination data because the API is not running.',error)}}
...
function startPlannerVideoFlow(){
  ...
  const setPlace=placeId=>{const file=plannerVideoByDestination[placeId]||'india-tourism-hero.mp4',nextPlaylist=[0,5,10,15,20,25].map(start=>({file,start})),request=++changeRequest;
